...
- Опционально на странице установщика: скачивание MSI **Java 21** (пин Temurin на GitHub) через **PowerShell `Invoke-WebRequest`** (не `NSISdl` — встроенный NSISdl не умеет HTTPS) и установка **только при явной галочке**, **`msiexec /qn`** (без изменения `JAVA_HOME`, если пользователь не выбрал опцию). URL закреплены в [`buildScript/temurin21-pinned.urls`](../buildScript/temurin21-pinned.urls) (зеркало [adoptium/temurin21-binaries](https://github.com/adoptium/temurin21-binaries) на GitHub, не adoptium.net). Перед JNI-скачиванием CI гоняет [`buildScript/ci/verify_temurin21_urls.sh`](../buildScript/ci/verify_temurin21_urls.sh).
