...
1. Закоммитьте или спрячьте локальные правки; основная линия разработки — **`main`** (default на `origin` / `github`); при необходимости фичи в отдельной `feature/...`.
...
5. После успешного gate — cherry-pick/merge интеграционной ветки в **`main`**, затем `git push origin main` и при необходимости `git push github main`.
